			...
			FMLCommonHandler.instance().getWorldThread(ctx.netHandler).addScheduledTask(new Runnable(){
				...
				public void run(){
					...
					if(vehicle!=null){
						if(vehicle.towedVehicle != null){
						...
						}else if(vehicle.packComponent.pack.motorized.hitchPos != null){
							for(Entity entity : vehicle.world.loadedEntityList){
								if(entity instanceof EntityVehicleF_Ground){
									...
									if(testVehicle.packComponent.pack.motorized.hookupPos != null){
										for(String hitchType : vehicle.packComponent.pack.motorized.hitchTypes){
											if(hitchType.equals(testVehicle.packComponent.pack.motorized.hookupType)){
												//Make sure clients hitch vehicles that the server sees.  Little more lenient here.
												Vec3d hitchOffset = new Vec3d(vehicle.packComponent.pack.motorized.hitchPos[0], vehicle.packComponent.pack.motorized.hitchPos[1], vehicle.packComponent.pack.motorized.hitchPos[2]);
												...
												Vec3d hookupOffset = new Vec3d(testVehicle.packComponent.pack.motorized.hookupPos[0], testVehicle.packComponent.pack.motorized.hookupPos[1], testVehicle.packComponent.pack.motorized.hookupPos[2]);
