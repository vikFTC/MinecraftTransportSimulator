...
import minecrafttransportsimulator.packs.objects.PackObjectInstrument.PackInstrumentComponent;
...
public abstract class RenderInstruments{
	...
	public static void drawInstrument(EntityVehicleE_Powered vehicle, PackComponentInstrument instrumentComponent, boolean hud, byte engineNumber){
		//First get the appropriate texture file for this vehicle/instrument combination.
		if(!instrumentTextureSheets.containsKey(vehicle.packComponent.pack.general.type)){
			instrumentTextureSheets.put(vehicle.packComponent.pack.general.type, new HashMap<String, ResourceLocation>());
		}
		if(!instrumentTextureSheets.get(vehicle.packComponent.pack.general.type).containsKey(instrumentComponent.name)){			
			instrumentTextureSheets.get(vehicle.packComponent.pack.general.type).put(instrumentComponent.name, new ResourceLocation(MTS.MODID, "textures/instruments/" + vehicle.packComponent.pack.general.type + ".png"));
		}
		textureManager.bindTexture(instrumentTextureSheets.get(vehicle.packComponent.pack.general.type).get(instrumentComponent.name));
		...
		float textureUStart = (instrumentComponent.pack.general.textureXSectorStart - 1)/8F;
		float textureVStart = (instrumentComponent.pack.general.textureYSectorStart - 1)/8F;
		...
		byte currentLayer = 0;
		for(PackInstrumentComponent component : instrumentComponent.pack.components){
			GL11.glPushMatrix();
	...
	private static double getVariableValue(EntityVehicleE_Powered vehicle, String variable, byte engineNumber){
		switch(variable){
			...
			case("fuel"): return vehicle.fuel/vehicle.packComponent.pack.motorized.fuelCapacity*100F;
			case("rpm"): return vehicle.getEngineByNumber(engineNumber) != null ? (vehicle.getEngineByNumber(engineNumber).packComponent.pack.engine.maxRPM < 15000 ? vehicle.getEngineByNumber(engineNumber).RPM : vehicle.getEngineByNumber(engineNumber).RPM/10D) : 0;
			case("rpm_max"): return vehicle.getEngineByNumber(engineNumber) != null ? (vehicle.getEngineByNumber(engineNumber).packComponent.pack.engine.maxRPM < 15000 ? APartEngine.getSafeRPMFromMax(vehicle.getEngineByNumber(engineNumber).packComponent.pack.engine.maxRPM) : APartEngine.getSafeRPMFromMax(vehicle.getEngineByNumber(engineNumber).packComponent.pack.engine.maxRPM)/10D) : 0;
