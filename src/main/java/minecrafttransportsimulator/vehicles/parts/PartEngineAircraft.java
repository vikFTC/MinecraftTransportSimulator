...
public class PartEngineAircraft extends APartEngine{
	...
	public PartEngineAircraft(EntityVehicleE_Powered vehicle, PackComponentPart packComponent, PackPart vehicleDefinition, NBTTagCompound dataTag){
	...
	public void updatePart(){
		...
		if(state.running){
			...
			if(propeller != null){
				double propellerForcePenalty = (propeller.packComponent.pack.propeller.diameter - 75)/(50*this.packComponent.pack.engine.fuelConsumption - 15);
				double propellerFeedback = (0.0254*Math.abs(propeller.currentPitch)*RPM*packComponent.pack.engine.gearRatios[0]/60/20 - vehicle.velocity + propellerForcePenalty)*50;
			...
			if(propeller != null){
				RPM = Math.max(RPM + (vehicle.velocity - 0.0254*Math.abs(propeller.currentPitch)*RPM*packComponent.pack.engine.gearRatios[0]/60/20)*15 - 10, 0);
			}else{
		...
		engineDriveshaftRotation += 360D*RPM/1200D*packComponent.pack.engine.gearRatios[0];
	}
	...
	public double getForceOutput(){
		if(propeller != null && Math.abs(propeller.currentPitch) > 20 && state.running){
			...
			double effectivePitchVelocity = 0.0254D*propeller.currentPitch*RPM*packComponent.pack.engine.gearRatios[0]/60D;
			//Multiply by a factor to get the true effective pitch velocity.  This is slightly higher than ideal.
			effectivePitchVelocity *= (1D*propeller.currentPitch/propeller.packComponent.pack.propeller.diameter + 0.2D)/(1D*propeller.currentPitch/propeller.packComponent.pack.propeller.diameter);
			if(effectivePitchVelocity != 0){
				...
				double thrust = vehicle.airDensity*Math.PI*Math.pow(0.0254*propeller.packComponent.pack.propeller.diameter/2D, 2)*
						(effectivePitchVelocity*effectivePitchVelocity - effectivePitchVelocity*currentPitchVelocity)*
						Math.pow(propeller.packComponent.pack.propeller.diameter/2D/Math.abs(propeller.currentPitch) + propeller.packComponent.pack.propeller.numberBlades/1000D, 1.5)/400D;
